...
// --------------------- Connect DB ---------------------
func ConnectionDB() {
	// ถ้าต้องการ persistent บน Render: ตั้ง ENV DATA_DIR เป็น /var/data (หลัง Add Disk)
	// ถ้าไม่ตั้ง จะใช้โฟลเดอร์ปัจจุบัน (ephemeral บน Render)
	...
	dbPath := filepath.Join(dataDir, "work.db")
	// เปิด WAL + busy_timeout สำหรับ concurrency และลด busy error
	dsn := fmt.Sprintf("file:%s?cache=shared&_pragma=busy_timeout(5000)&_pragma=journal_mode(WAL)", dbPath)
...
// findUserIDByUsername: ดึง ID จาก username เพื่อลดการอ้าง ID ตายตัว
